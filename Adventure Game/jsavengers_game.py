...
def intro():
    ...
    print_pause("To your left, there's a Quinjet that looks like it might have "
                "some valuable loot inside.")
...
def fight(villain, weapon):
    ...
    # Assign random win chances for other weapons between 40% and 80%
    random_chances = {
    ...
    # Update weapon_win_chances with the random chances
    weapon_win_chances.update(random_chances)

    # Get the winning probability for the current weapon
    win_probability = weapon_win_chances.get(weapon, 0.4)  # Default to blaster's win rate if not found

    print_pause(f"As {villain} prepares to attack, you ready your {weapon}.")
    
    # Randomly determine if the player wins based on the weapon's win probability
    if random.random() < win_probability:
        print_pause(f"The {weapon} crackles with energy as you charge at {villain}.")
...
def find_new_weapon(current_weapon, location):
    if location == 'tower':
        all_weapons = ['Captain America\'s Shield', 'Black Widow\'s Batons', 'Iron Man\'s Repulsors']
    else:
        all_weapons = ['Thor\'s Hammer', 'Hawkeye\'s Bow', 'Ant-Man\'s Suit']
    
    available_weapons = [weapon for weapon in all_weapons if weapon != current_weapon]
...
def avengers_tower(villain, weapon, tower_visits):
    ...
    # If it's the first visit, force an event to happen
    if tower_visits == 0:
        tower_event = random.choice(['weapon', 'villain'])
    else:
        # Increase likelihood of finding a villain with each visit
        villain_probability = min(0.3 + (0.2 * tower_visits), 1.0)  # Start with 30%, increase by 20% each visit
        if random.random() > villain_probability:
            print_pause("Nothing has changed, but you hear noise near the Quinjet.")
            return weapon  # Return current weapon unchanged
        else:
            tower_event = random.choice(['weapon', 'villain'])
    
    # Handle the event (villain or weapon)
    if tower_event == 'villain':
        ...
        choice = valid_input("Would you like to (1) fight or (2) retreat to the "
                             "courtyard? ", ['1', '2'])
        if choice == '1':
            result = fight(villain, weapon)
            if result:  # If the player wins, end the game
                return True  # Player won
            else:
                return False  # Player lost
        else:
        ...
        choice = valid_input(f"Would you like to swap your {weapon} for {new_weapon}? (y/n)\n", ['y', 'n'])
        if choice == 'y':
            print_pause(f"You discard your {weapon} and take {new_weapon} with you.")
...
def quinjet(villain, weapon, quinjet_visits):
    print_pause("You cautiously enter the Quinjet, searching for something useful.")
    ...
    if quinjet_visits == 0:
        ...
        # Increase likelihood of finding a villain with each visit
        villain_probability = min(0.3 + (0.2 * quinjet_visits), 1.0)  # Start with 30%, increase by 20% each visit
        if random.random() > villain_probability:
            print_pause("Nothing has changed, but you hear noise near Avengers Tower.")
            return weapon  # Return current weapon unchanged
        else:
            quinjet_event = random.choice(['weapon', 'villain'])
    ...
    if quinjet_event == 'villain':
        print_pause(f"But instead, {villain} is waiting for you inside the Quinjet!")
        print_pause(f"{villain} launches an attack!")
...
def shield_facility():
    ...
    # Track how many times the player has visited the tower or the Quinjet
    tower_visits = 0
    quinjet_visits = 0
    ...
    while True:
        ...
        if choice == '1':
            ...
            tower_visits += 1  # Increase visit count for the tower
            if result is False:
                return False  # Player was defeated
            elif result is True:
                return True  # Player won the game
            else:
                weapon = result  # Update weapon after the encounter
        else:
            result = quinjet(villain, weapon, quinjet_visits)
            quinjet_visits += 1  # Increase visit count for the Quinjet
            if result is False:
                return False  # Player was defeated
            elif result is True:
                return True  # Player won the game
        ...
        # Return to the courtyard after Quinjet or Tower events
        print_pause("You return to the facility courtyard, preparing for your next move.")
